...
import devStockLevelOnCategory from '../custom/stock-level-on-category';

export default function () {
    const TOP_STYLING = 'top:' + $(".header").height() + 'px';
    const $quickSearchResults = $('.quickSearchResults');
    const $quickSearchForms = $('[data-quick-search-form]');
    const $quickSearchExpand = $('#quick-search-expand');
    const $quickSearchExpand1 = $('#quick-search-expand-1');
    const $searchQuery = $quickSearchForms.find('[data-search-quick]');
    const stencilDropDownExtendables = {
        hide: () => {
            $quickSearchExpand.attr('aria-expanded', false);
            $quickSearchExpand1.attr('aria-expanded', false);
            $searchQuery.trigger('blur');
        },
        show: (event) => {
            $quickSearchExpand.attr('aria-expanded', true);
            $quickSearchExpand1.attr('aria-expanded', true);
